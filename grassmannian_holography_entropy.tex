...
% -------- Left panel: Positive geometry facet in Gr^+(2,4) --------
\begin{minipage}{0.42\linewidth}
\centering
\begin{tikzpicture}[scale=1.1, line join=round, line cap=round, font=\small]
  % Cell (drawn as a rectangle for clarity)
  \fill[blue!6] (0,0)--(3,0)--(3,2)--(0,2)--cycle;
  \draw[thick,blue!50!black] (0,0)--(3,0)--(3,2)--(0,2)--cycle;

  % Highlight one facet (soft boundary)
  \draw[line width=1.3pt,red!70!black] (0,0)--(0,2);
  \node[rotate=90,anchor=south,red!70!black] at (-0.08,1) {$c_4=0$};

  % Other facet labels
  \node[below] at (1.5,0) {$c_1=0$};
  \node[above] at (1.5,2) {$c_3=0$};
  \node[right] at (3.0,1) {$c_2=0$};

  % Interior annotation
  \node at (1.5,1) {$\mathrm{Gr}^+(2,4)$};
  \node[red!70!black,anchor=west] at (0.1,1.65) {\(\omega^{-1}\) (soft)};
\end{tikzpicture}

\vspace{3pt}
\textit{(a) Facet in positive geometry}
\end{minipage}
\hspace{0.04\linewidth}
% -------- Middle arrow label --------
\raisebox{1.1cm}{\(\xrightarrow{\;\;\text{Mellin (energy)}\;\;}\)}
\hspace{0.04\linewidth}
% -------- Right panel: Celestial pole at Delta = 1 --------
\begin{minipage}{0.42\linewidth}
\centering
\begin{tikzpicture}[scale=1.1, line join=round, line cap=round, font=\small]
  % Horizontal axis for Delta
  \draw[->,thick] (-0.2,0)--(3.6,0) node[below right] {$\Delta$};
  ...
  % Simple pole at Delta = 1
  \draw[red!70!black, line width=1.3pt] (1,-0.2)--(1,1.4);
  \node[red!70!black, anchor=west] at (1.05,1.1) {simple pole at \(\Delta=1\)};
  \node[anchor=west] at (1.05,0.65) {\(\Gamma(\Delta-1)\sim\dfrac{1}{\Delta-1}\)};

  % Context label
  \node[anchor=west] at (0.1,1.8) {celestial correlator \(\widetilde{\mathcal A}_n(\Delta_i,z_i,\bar z_i)\)};
\end{tikzpicture}

\vspace{3pt}
\textit{(b) \(\Delta{=}1\) pole in celestial space}
\end{minipage}

\caption{Facet \(\to\) celestial pole. \textbf{(a)} A codimension-one facet \(c_4{=}0\) in the positive geometry \(\mathrm{Gr}^+(2,4)\) produces the universal leading soft scaling \(\omega^{-1}\). \textbf{(b)} Under the Mellin (energy) transform, \(\omega^{-1}\mapsto \Gamma(\Delta{-}1)\), giving a simple pole at \(\Delta{=}1\) in the celestial correlator \(\widetilde{\mathcal A}_n(\Delta_i,z_i,\bar z_i)\), i.e. the celestial Ward identity.}
\label{fig:facet-to-pole}
\end{figure}
